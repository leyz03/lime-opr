...
**日期：** 2026-05-16 | **关联实验：** [EXP-011](RECORD.md#exp-011--收敛曲线--迭代敏感性)
...
2. **成本分布重尾、方差大。** C_p=30 的 lost-demand 惩罚 + Dirichlet OD + 单 hot spot，少数坏需求实现即可把单条样本拉到很负（冻结的 LD 曾出现 sim_μ=−98）。重尾下 nsim=300 太少，标准误压不下。
...
| **增大 nsim**（300 → 2000+） | 标准误 ∝ 1/√n，CI 半宽缩到 ±4 以下 | 中，仿真时间线性增加 |
| **公共随机数 / Antithetic / 控制变量** | CRN 把策略差异与场景噪声解耦；控制变量用确定性近似（如 myopic 解）做基准 | 中，需改 simulate |
| **降低成本分布尾部** | 给 C_p / 价格上界设更紧范围、对极端需求实现做截断或重要性采样 | 中，会改变问题语义需谨慎 |
| **报告分位数而非均值** | 中位数 / 5%-95% 带对重尾更稳健 | 低 |
